...
$(function() {
    ...
    $.get('/ip', function(data) {
        ...
        let ipc = data.country;
        if (ipc !== 'China')
            popInfo('unsupported', '所在地区暂不支持');
    });
...
function startSyncLyric() {
    ...
    lrcSyncTaskId = setInterval(function() {
        ...
        for (let i = 0; i < $lrcL.length; i++) {
            ...
            if (i === $lrcL.length - 1) pPoi = i;
        }
        if (rp === $lrcL.length && pPoi === -1) return;
        for (let i = 0; i < $tlrcL.length; i++) {
            let ip = $($tlrcL[i]);
            if (ip.attr('data-time') > curPlayingTime) {
                tpPoi = i - 1;
                break;
            }
            if (i === $tlrcL.length - 1) tpPoi = i;
        }
        let p = $($lrcL[pPoi]);
        let tp = $($tlrcL[tpPoi]);
        if (rp !== pPoi) {
            if (rp !== -1) $($lrcL[rp]).removeClass('lrc_cur');
            if (rtp !== -1) $($tlrcL[rtp]).removeClass('lrc_cur');
            rp = pPoi;
            p.addClass('lrc_cur');
